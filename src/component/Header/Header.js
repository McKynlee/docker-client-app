...
class Header extends Component {
  ...
  render() {
    ...
    const navLinks = navConfig
      .filter((navData) => navData.mainNav)
      .map((navData, index) => {
        return (
          <li key={index}>
            <Link className="navLink" to={navData.path}>
              {navData.name}
            </Link>
          </li>
        );
          ...
          <nav className="nav">
            ...
            <div className={navPaneStyles}>
              <div className="nav-pane-hd">
                <button className="btnClose" onClick={this.onToggleMobileNav}>
                  Close
                </button>
              </div>
              <ul className="nav-pane-list">{navLinks}</ul>
            </div>
          </nav>
        </div>
      </header>
